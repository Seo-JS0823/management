// empDelete : 부서에 담당자가 존재하는 경우 경고창 출력
const empDeleteEl = document.querySelector('[id=empDelete]');

if(empDeleteEl != null) {
	alert(empDeleteEl.value);
}

const joinBtn = document.querySelector('[id=join]');

joinBtn.addEventListener('click', (e) => {
	const name		= document.querySelector('[id=name]');
	const phone		= document.querySelector('[id=phone]');
	const part		= document.querySelector('[id=part]');
	const work		= document.querySelector('[id=work]');
	const posi		= document.querySelector('[id=posi]');
	const empDate	= document.querySelector('[id=emp-date]');
	const email		= document.querySelector('[id=email]');
	const address	= document.querySelector('[id=address]');
	const birth		= document.querySelector('[id=birth]');
	
	if(name.value == '') {
		alert('관리자 성함을 입력하세요.');
		e.preventDefault();
		return false;
	}
	
	const phoneReg = /^0\d{1,2}-\d{3,4}-\d{4}$/;
	
	if(!phoneReg.test(phone.value)) {
		alert('010-0000-0000 형식으로 전화번호를 입력하세요.');
		e.preventDefault();
		return false;
	}
	
	if(part.value == '') {
		alert('부서를 선택하세요.');
		e.preventDefault();
		return false;
	}
	
	if(work.value == '') {
		alert('근무 형태를 선택하세요.');
		e.preventDefault();
		return false;
	}
	
	if(posi.value == '') {
		alert('직급을 선택하세요.');
		e.preventDefault();
		return false;
	}
	
	if(empDate.value == '') {
		alert('입사일을 선택하세요.');
		e.preventDefault();
		return false;
	}
	
	if(email.value != '') {
		const emailReg = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/;
		
		if(!emailReg.test(email.value)) {
			alert('올바른 이메일 형식으로 입력하세요.');
			e.preventDefault();
			return false;
		}
	}
	
	if(address.value == '') {
		alert('주소를 검색하세요.');
		e.preventDefault();
		return false;
	}
	
	if(birth.value == '') {
		alert('생년월일을 선택하세요.');
		e.preventDefault();
		return false;		
	}
	
	const gender = document.querySelector('input[name="gender"]:checked');
	
	if(confirm('성별은 ' + gender.value + ' 이 맞습니까? 수정할 수 없습니다.')) {
		return true;
	} else {
		e.preventDefault();
		return false;		
	}
	
});